...
async function testUserCalendar() {
  ...
  try {
    const res = await calendar.events.list({
      calendarId: 'tayaramadam25@gmail.com',
      timeMin: new Date('2026-03-18T00:00:00Z').toISOString(),
      maxResults: 10,
      singleEvents: true,
      orderBy: 'startTime',
    });
    console.log('Successfully accessed user calendar!');
    const events = res.data.items;
    if (!events || events.length === 0) {
      console.log('No upcoming events found on user calendar.');
    } else {
    ...
    console.error('Error accessing user calendar:', error.message);
  }
}
