
import { google } from 'googleapis';

async function fixAclFinal() {
  const clientEmail = 'calendar-bot@rotem-whatsapp-bot.iam.gserviceaccount.com';
  const privateKey = `-----BEGIN PRIVATE KEY-----\\nWJXLDiiz9bBymDMhdmFnroxP\n-----END PRIVATE KEY-----\n`.replace(/\\n/g, '\n');

  const auth = new google.auth.JWT({
    email: clientEmail,
    key: privateKey,
    scopes: ['https://www.googleapis.com/auth/calendar'],
  });

  const calendar = google.calendar({ version: 'v3', auth });

  const targetEmails = ['tayaramadam25@gmail.com', 'a.k.cleaningg@gmail.com'];

  for (const email of targetEmails) {
    try {
      console.log(`Setting ACL for ${email}...`);
      await calendar.acl.insert({
        calendarId: clientEmail, // Use full email instead of 'primary'
        requestBody: {
          role: 'reader',
          scope: { type: 'user', value: email },
        },
      });
      console.log(`Success for ${email}`);
    } catch (e) {
      console.error(`Failed for ${email}:`, e.message);
    }
  }

  try {
     console.log('Attempting to make calendar public (freeBusyReader)...');
     await calendar.acl.insert({
       calendarId: clientEmail,
       requestBody: {
         role: 'freeBusyReader',
         scope: { type: 'default' },
       },
     });
     console.log('Successfully made calendar public!');
  } catch (e) {
     console.error('Failed to make public:', e.message);
  }
}

fixAclFinal();
