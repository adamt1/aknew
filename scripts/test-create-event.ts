...
async function testCreateEvent() {
  ...
  const targetCalId = 'a.k.cleaningg@gmail.com';

  try {
    console.log(`Attempting to create test event on ${targetCalId}...`);
    const res = await calendar.events.insert({
      calendarId: targetCalId,
      requestBody: {
        summary: 'Test Event from Rotem Bot',
        start: { dateTime: new Date().toISOString() },
        end: { dateTime: new Date(Date.now() + 60000).toISOString() },
      },
    });
    console.log('Successfully created event! ID:', res.data.id);
  } catch (error: any) {
